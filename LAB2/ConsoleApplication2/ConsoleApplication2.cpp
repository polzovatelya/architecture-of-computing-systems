...
double octalToDecimal(const string& octalStr);
	...
	if (!fin) {
		...
		if (!fout) {
			...
			while (!fin.eof()) {
				...
				double decimalNum,decimalNum1;
				if (s1[0] == '-') {
					decimalNum = octalToDecimal(s1.erase(0, 1));
					decimalNum = -decimalNum;

				}
				else {
					decimalNum = octalToDecimal(s1);
				}
				if (s2[0] == '-') {
					decimalNum1 = octalToDecimal(s2.erase(0, 1));
					decimalNum1 = -decimalNum1;

				}
				else {
					decimalNum1 = octalToDecimal(s2);
				}
				fout << " Проверка = " << to_string(decimalNum + decimalNum1) << "\n";
